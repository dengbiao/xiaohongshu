...
import { FiImage, FiSave, FiDownload, FiPlusCircle, FiTrash2, FiEdit2, FiEye, FiEdit, FiCheckCircle } from 'react-icons/fi';
...
const Watermark: React.FC = () => {
  ...
  const [showAppliedMessage, setShowAppliedMessage] = useState<number | null>(null);
  const [showSavedMessage, setShowSavedMessage] = useState<number | null>(null);
  const [initialSettings, setInitialSettings] = useState<any>(null);
  const [hasChanged, setHasChanged] = useState(false);
  
  const { fetchedItems } = useFetchStore();
  const { watermarkSettings, updateSettings, savedWatermarks, addWatermark, deleteWatermark } = useWatermarkStore();
  ...
  const updateCurrentPreset = () => {
    ...
    // 找到当前选中的预设
    const currentPreset = savedWatermarks.find(preset => preset.id === activePreset);
    if (!currentPreset) {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      >
        ...
        <p className="text-gray-600 mb-8">为图片添加自定义水印，保护您的内容版权</p>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* 左侧 - 图片选择 */}
          <div className={`lg:col-span-1 ${previewMode ? 'hidden' : 'block'}`}>
            <div className="bg-white rounded-xl shadow-lg p-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">选择图片</h2>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 mt-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold text-gray-800">水印预设</h2>
                <button
                  ...
                  savedWatermarks.map(preset => (
                    ...
                    >
                      <div className="flex-grow">
                        ...
                        <p className="text-gray-500 text-sm">{preset.settings.text}</p>
                      </div>
                      <div className="flex items-center">
                        ...
                        {activePreset === preset.id && !showAppliedMessage && !showSavedMessage && (
          ...
          <div className={`${previewMode ? 'lg:col-span-1' : 'lg:col-span-1'}`}>
            <div className={`bg-white rounded-xl shadow-lg p-6 ${previewMode ? 'hidden' : 'block'}`}>
              <h2 className="text-xl font-semibold text-gray-800 mb-4">水印设置</h2>
              
              <div className="space-y-4">
                <div>
                  <label className="block text-gray-700 font-medium mb-2">水印文字</label>
                  <input
                    type="text"
                    name="text"
                ...
                <div>
                  <label className="block text-gray-700 font-medium mb-2">字体</label>
                  <select
                    name="font"
                ...
                <div>
                  <label className="block text-gray-700 font-medium mb-2">文字颜色</label>
                  <input
                    type="color"
              ...
              <div className="mt-6">
                ...
                >
                  ...
                  {activePreset 
                    ? `更新"${savedWatermarks.find(p => p.id === activePreset)?.name || '当前'}"预设` 
                    : '请先选择预设'}
                </button>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 h-full">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">
                ...
                {activePreset !== null && (
                  <span className="text-sm font-normal text-pink-600 ml-2">
                    (使用预设: {savedWatermarks.find(p => p.id === activePreset)?.name})
                  </span>
                )}
              </h2>
                ...
                <div>
                  ...
                  >
                    <img 
                      ...
                      alt="预览图片" 
                      className={`w-full ${previewMode ? 'h-full object-contain' : 'h-auto'}`}
                    />
                    
                    {/* 水印层 */}
                    <div className="absolute inset-0 flex flex-wrap pointer-events-none">
                      {Array.from({ length: watermarkSettings.density * 5 }).map((_, index) => (
                        <div 
                          key={index}
              ...
              {previewMode && selectedImage && (
                <div className="mt-4 text-center text-gray-500 text-sm">
                  <p>预览模式下，您可以更清晰地查看水印效果。点击"编辑模式"可返回设置界面。</p>
                </div>
              )}
            </div>
          </motion.div>
